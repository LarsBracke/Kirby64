...
void SoftwareSkinningScene_2::Update()
{
	...
	for (int index = 0; index < m_SkinnedVertices.size() / 2.f; ++index) /*bone0*/
	{
		VertexSoftWareSkinned& v{ m_SkinnedVertices[index] };

		const XMVECTOR originalVertex = XMLoadFloat3(&v.originalVertex.Position);
		const XMVECTOR transformedVertex = XMVector3TransformCoord(originalVertex, boneTransform0);

		XMStoreFloat3(&v.transformedVertex.Position, transformedVertex);
	}
	for (int index = static_cast<int>(m_SkinnedVertices.size() / 2.f); index < m_SkinnedVertices.size(); ++index) /*bone1*/
	{
		VertexSoftWareSkinned& v{ m_SkinnedVertices[index] };

		const XMVECTOR originalVertex = XMLoadFloat3(&v.originalVertex.Position);
		const XMVECTOR transformedVertex = XMVector3TransformCoord(originalVertex, boneTransform1);
