...
void Character::Update(const SceneContext& sceneContext)
{
	if (m_pCameraComponent->IsActive())
	{
		...
		XMFLOAT2 move{ };
		const XMFLOAT2 leftThumbStickPos = sceneContext.pInput->GetThumbstickPosition();
		const XMFLOAT2 rightThumbStickPos = sceneContext.pInput->GetThumbstickPosition(false);
		...
		const POINT mouseMovement = sceneContext.pInput->GetMouseMovement();
		...
		if (sceneContext.pInput->IsMouseButton(InputState::down, 1))
		{
			...
			const XMFLOAT3 currentRot = MathHelper::QuaternionToEuler(pTransform->GetRotation());
			pTransform->Rotate(m_TotalPitch * m_MouseSensitivity, m_TotalYaw * m_MouseSensitivity, currentRot.z);
		}
		...
		if (MovementDetected(sceneContext))
		{
			XMVECTOR result = (move.x * right) + (move.y * forward);
			result = XMVector3Normalize(result);
			XMStoreFloat3(&m_CurrentDirection, result);
		...
		const XMVECTOR currentDirection = XMLoadFloat3(&m_CurrentDirection);
		XMVECTOR result = currentDirection * m_MoveSpeed;
		XMFLOAT3 horizontalVelocity{ };
		XMStoreFloat3(&horizontalVelocity, result);
		...
		PxVec3 origin = PhysxHelper::ToPxVec3(pTransform->GetPosition());
		...
		result = totalVelocity * sceneContext.pGameTime->GetElapsed();
		XMFLOAT3 displacement{ };
		XMStoreFloat3(&displacement, result);

		ControllerComponent* pControllerComponent = GetComponent<ControllerComponent>();
...
bool Character::MovementDetected(const SceneContext& sceneContext) const
{
	const InputManager* pInput = sceneContext.pInput;
	const bool movement =
		pInput->IsActionTriggered(m_CharacterDesc.actionId_MoveForward) ||
		pInput->IsActionTriggered(m_CharacterDesc.actionId_MoveBackward) ||
		pInput->IsActionTriggered(m_CharacterDesc.actionId_MoveLeft) ||
		pInput->IsActionTriggered(m_CharacterDesc.actionId_MoveRight);
