...
void SpriteRenderer::UpdateBuffer(const SceneContext& sceneContext)
{
	...
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		const HRESULT result = sceneContext.d3dContext.pDevice->CreateBuffer(&bufferDesc, nullptr, &m_pVertexBuffer);
		HANDLE_ERROR(result);
...
;		// Next you will need to use the device context to map the vertex buffer to the mapped resource
		sceneContext.d3dContext.pDeviceContext->Map(m_pVertexBuffer, NULL, D3D11_MAP_WRITE_NO_OVERWRITE, NULL, &mappedSubResource );
		...
		{
			const VertexSprite* pSource = &m_Sprites[index];
			VertexSprite* pDestination = &static_cast<VertexSprite*>(mappedSubResource.pData)[index * sizeof(VertexSprite)];

			std::memcpy(pDestination, pSource, sizeof(VertexSprite));
		}

		// unmap the vertex buffer
		sceneContext.d3dContext.pDeviceContext->Unmap(m_pVertexBuffer, NULL);
	}
}
