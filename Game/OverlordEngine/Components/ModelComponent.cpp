...
void ModelComponent::ShadowMapDraw(const SceneContext& sceneContext)
{
	...
	{
		const XMFLOAT4X4 world = GetGameObject()->GetTransform()->GetWorld();
		if (m_pAnimator != nullptr)
			ShadowMapRenderer::Get()->DrawMesh(sceneContext, m_pMeshFilter, world, m_pAnimator->GetBoneTransforms());
		else
			ShadowMapRenderer::Get()->DrawMesh(sceneContext, m_pMeshFilter, world);
	}
}
